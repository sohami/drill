...
public class LateralJoinBatch extends AbstractBinaryRecordBatch<LateralJoinPOP> implements LateralContract {
  ...
  private JoinBatchMemoryManager lateralBatchMemoryManager;
  ...
  public LateralJoinBatch(LateralJoinPOP popConfig, FragmentContext context,
    ...
    lateralBatchMemoryManager = new JoinBatchMemoryManager(configOutputBatchSize, left, right);
  }
  ...
  private void updateMemoryManager(int inputIndex) {
    final int previousRowWidth = lateralBatchMemoryManager.getOutgoingRowWidth();
    lateralBatchMemoryManager.update(inputIndex);

    // Calculate memory used so far.
    // For cases where all the previous input were consumed and send with previous output batch. But now we are building
    // a new output batch with new incoming then previousRowWidth will not cause any problem since outputIndex will be 0
    final long memoryConsumed = outputIndex * previousRowWidth;

    // Calculate remaining memory in current output batch
    final int configOutputBatchSize = lateralBatchMemoryManager.getOutputBatchSize();
    final long remainingMemory = Math.max(configOutputBatchSize - memoryConsumed, 0);

    // Calculate number of rows that can be inserted into current output batch and update the maxOutputCount
    final int currentRowWidth = lateralBatchMemoryManager.getOutgoingRowWidth();
    final int numOutputRowsRemaining = RecordBatchSizer.safeDivide(remainingMemory, currentRowWidth);

    if (useMemoryManager) {
      maxOutputRowCount = outputIndex + numOutputRowsRemaining;
    }
  }
}
