  ...
  public void authenticate(String user, String password) throws UserAuthenticationException {
    for (String file : profiles) {
      ...
      try {
        ...
        logger.error("PAM auth failed for user: " + user + " against " + file + " profile. " +
            "Exception: " + ex.getMessage());
        throw new UserAuthenticationException("PAM auth failed for user: " + user);
      } finally {
        if (pam != null) {
          pam.dispose();
        }
      ...
      if (logger.isTraceEnabled()) {
        ...
        logger.trace("PAM authentication was successful for user: " + unixUser.getUserName() + " using profile: " +
            file);
      }
    }
  }
