...
public class RMBlobManagerTest extends DrillTest {
  ...
  public void testRMStateBlobSerDe() throws Exception {
    ...
    try (ClusterFixture cluster = fixtureBuilder.build()) {
      ...
      Map<String, Object> queueConfigValues = new HashMap<>();
      queueConfigValues.put(QueryQueueConfigImpl.MAX_QUERY_MEMORY_PER_NODE_KEY, "8192K");

      Config queueConfig = ConfigFactory.empty().withValue("queue",
        ConfigValueFactory.fromMap(queueConfigValues));

      QueryQueueConfig leafQueue1 = new QueryQueueConfigImpl(queueConfig.getConfig("queue"), "queue1", null);
      QueryQueueConfig leafQueue2 = new QueryQueueConfigImpl(queueConfig.getConfig("queue"), "queue2", null);
      QueryQueueConfig leafQueue3 = new QueryQueueConfigImpl(queueConfig.getConfig("queue"), "queue3", null);

      List<QueryQueueConfig> leafQueues = new ArrayList<>();
      leafQueues.add(leafQueue1);
      leafQueues.add(leafQueue2);
      leafQueues.add(leafQueue3);

      List<String> drillUUID = new ArrayList<>();
      drillUUID.add(UUID.randomUUID().toString());
      drillUUID.add(UUID.randomUUID().toString());
      drillUUID.add(UUID.randomUUID().toString());

      RMConsistentBlobStoreManager rmManager = new RMConsistentBlobStoreManager(context, leafQueues);

      Map<String, NodeResources> clusterStateValue = new HashMap<>();
      ...
      Map<String, String> queueLeadersValue = new HashMap<>();
      ...
      Map<String, NodeResources> foreman1Usage = new HashMap<>();
      foreman1Usage.put(drillUUID.get(1), new NodeResources(1000, 1));
      foreman1Usage.put(drillUUID.get(2), new NodeResources(2000, 1));

      Map<String, NodeResources> foreman2Usage = new HashMap<>();
      foreman2Usage.put(drillUUID.get(0), new NodeResources(1000, 1));
      foreman2Usage.put(drillUUID.get(2), new NodeResources(2000, 1));

      Map<String, NodeResources> foreman3Usage = new HashMap<>();
      foreman3Usage.put(drillUUID.get(0), new NodeResources(1000, 1));
      foreman3Usage.put(drillUUID.get(1), new NodeResources(2000, 1));


      ForemanResourceUsage foreman1 = new ForemanResourceUsage(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION,
        foreman1Usage, 1);
      ForemanResourceUsage foreman2 = new ForemanResourceUsage(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION,
        foreman2Usage, 2);
      ForemanResourceUsage foreman3 = new ForemanResourceUsage(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION,
        foreman3Usage, 3);

      Map<String, ForemanResourceUsage> formemanQueueUsageValues = new HashMap<>();
      ...
      RMStateBlob clusterState = new ClusterStateBlob(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION, clusterStateValue);
      RMStateBlob queueLeaders = new QueueLeadershipBlob(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION, queueLeadersValue);
      RMStateBlob foremanQueueUsage = new ForemanQueueUsageBlob(RMConsistentBlobStoreManager.RM_STATE_BLOB_VERSION,
        formemanQueueUsageValues);
      Map<String, RMStateBlob> blobsToSerialize = new HashMap<>();
      blobsToSerialize.put(ClusterStateBlob.NAME, clusterState);
      blobsToSerialize.put(QueueLeadershipBlob.NAME, queueLeaders);

      for (QueryQueueConfig leafQueue : leafQueues) {
        String blobName = ForemanQueueUsageBlob.NAME + "_" + leafQueue.getQueueName();
        blobsToSerialize.put(blobName, foremanQueueUsage);
      }

      final Map<String, String> serializedBlobs = rmManager.serializePassedInBlob(blobsToSerialize);
      final Map<String, RMStateBlob> deserializedBlobs = rmManager.deserializeRMStateBlobs(serializedBlobs);

      for (Map.Entry<String, RMStateBlob> blobEntry : deserializedBlobs.entrySet()) {
        RMStateBlob actualBlob = blobEntry.getValue();
